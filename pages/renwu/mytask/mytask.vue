	...
	<view class="content uni-scroll-view pos-r">
		...
		<view class="w100 pos-r">
			...
			<navigator url="../renwu-details/renwu-details" hover-class="navigator-hover" class="w100 pos-r">
				<view class="user-div bg-white bd-bottom pr-10 pt-15 pb-10" v-for="(list,index) in tableList">
					...
					<view class="f-l line-h">
						<view class="user-text mt-5">
							...
							<view class="f-14 f-all mt-5">{{list.requirement}}</view>
							<view class="f-14 f-all mt-5">佣金：{{list.commissionMoney}}金豆</view>
						</view>
	...
	export default {
		...
		methods: {
			...
			list(page, status) {
				...
				if (status) {
					json['map.key.status'] = status
				}
				this.post("api/app/task/single ", json, true)
					.then(data => {
						console.log(data);
						let that = this;
						let table = JSON.parse(JSON.stringify(data.content));
